...
class BacktestingEngine(object):
    ...
    #----------------------------------------------------------------------
    def __init__(self, bt_Config, eventEngine = None):
        """Constructor"""
        self.Rt_Dir = bt_Config['Rt_Dir']
        ...
        self.unsetpnl = 0
        self.networth = 0
        self.dailyinf = pd.DataFrame(columns=['Posmkv','Networth','Margin','Margpct','Leverage'])
        ...
        self.setStartDate(bt_Config['Start_Date'])
        self.setEndDate(bt_Config['End_Date'])
        self.symbol = bt_Config['Symbol']

        self.miniT = bt_Config['MiniT']
        self.load_param([self.symbol])
        self.set_varparam(self.symbol)
        self.setSlippage(bt_Config['Slippage'])
        pass


    #----------------------------------------------
    def load_param(self, varlist):
        Csv_Dir = self.Meta_Csv_Dir
        self.Variety_List = varlist[:]
    ...
    def set_tick_size(self, Csv_Dir, Col_Nam='TickSize', Index_Col=0):
        ...
        for x in self.Variety_List:
            ...
            pass
    #--------------------------------------------------------
    def set_varparam(self, var):
        self.rate = self.Transaction_Rate[var]  # 回测时假设的佣金比例（适用于百分比佣金）
        self.size = self.Multiplier[var]    # 合约大小，默认为1
        self.priceTick = self.Tick_Size[var]
        self.minDiff = self.priceTick
        self.fixCommission = self.Transaction_Fee[var]
        self.margin_rate = self.Margin_Rate[var]
    # --------------------------------------------------------
    ...
    # ----------------------------------------------------------------------
    def setTradeSymbol(self, symbol):
        """设置引擎的交易合约"""
        self.symbol = symbol
    ...
    def setSlippage(self, slippage):
        """设置滑点点数"""
        self.slippage = slippage * self.priceTick
    ...
    def runBacktesting(self):
        """运行回测"""
        self.capital = self.initCapital      # 更新设置期初资金
        # self.dailyinf = pd.DataFrame([[0, self.capital, 0, 0, 0]], index=['init'], columns=['Posmkv','Networth','Margin','Margpct','Leverage'])
        # 首先根据回测模式，确认要使用的数据类
        if self.mode == self.BAR_MODE:
            dataClass = CtaBarData
            func = self.newBar
        ...
        count_bars = 0
        # 在 vada0上回放数据做测试
        vada = self.strategy.vada0
        for dtm in vada.dat.index:
            data = dataClass()
            ds = vada.dat.loc[dtm,:]
            data.datetime = dtm
            for k in ds.index:
                if k in data.__dict__:
                    data.__dict__[k] = ds[k]
            func(data)
            count_bars += 1
            try:
                nxtdtmhh = vada.dat.index[count_bars][11:13]
            except:
                nxtdtmhh = '09'
            if dtm[11:13] =='15' and (nxtdtmhh =='09' or nxtdtmhh =='21'):
                self.endOfDay(dtm, ds['close'])

        self.output(u'回测区间：{0} -- {1}，数据量：{2}，回测耗时:{3}'
                    .format(self.dataStartDate.strftime('%Y%m%d'), self.dataEndDate.strftime('%Y%m%d'), count_bars,
                            str(datetime.now() - process_time)))

        self.output(u'数据回放结束')

    # ----------------------------------------------------------------------
    def loadDataFromMongo(self, vadas):
        """载入历史数据"""
        host, port, log = loadMongoSetting()
        self.dbClient = pymongo.MongoClient(host, port)
        for vdn, vada in vadas.iteritems():
            if not vada:
                continue
            print 'Load' + vdn+ ' from mongodb for ' + vada.var
            dbName = '_'.join(['Dom', vada.period])
            collection = self.dbClient[dbName][vada.var]
            vada.dat = pd.DataFrame()
            # 载入初始化需要用的数据
            flt = {'datetime': {'$gte': self.dataStartDate.strftime('%Y-%m-%d'),
                                '$lt': self.dataEndDate.strftime('%Y-%m-%d')}}
            self.dbCursor = collection.find(flt).sort('datetime', pymongo.ASCENDING)
            datas = list(self.dbCursor)
            if len(datas) > 0:
                vada.dat = pd.DataFrame(datas)
                vada.dat.drop(['_id'], axis=1, inplace=True)
                vada.dat.set_index('datetime', inplace=True)
                vada.indexs = vada.dat.index.tolist()
    # ----------------------------------------------------------------------
    def loadHistoryDataFromMongo(self):
        ...
        self.output(u'回测区间：{0} -- {1}，数据量：{2}，查询耗时:{3},回测耗时:{4}'
                    .format(self.dataStartDate.strftime('%Y%m%d'), self.dataEndDate.strftime('%Y%m%d'), count_ticks, str(datetime.now() - query_time),
                            str(datetime.now() - process_time)))
    ...
    def showBacktestingResult(self):
        ...
        fig_file_name1 = os.path.abspath(os.path.join(os.path.dirname(__file__), 'TestLogs',
                                                     'Fig1_{0}.png'.format(datetime.now().strftime('%Y%m%d_%H%M'))))
        fig = plt.gcf()
        fig.savefig(fig_file_name1)
        print (u'图表保存至：{0}'.format(fig_file_name1))
        #plt.show()
        #-----
        fig, (ax1, ax2) = plt.subplots(2, sharex=True)
        Networth = self.dailyinf.loc[:, ['Networth']]
        Leverage = self.dailyinf.loc[:, ['Leverage']]

        Networth.plot(ax=ax1, title='Networth')
        Leverage.plot(ax=ax2, title='Leverage')
        xdate = [itime for itime in Networth.index]
        ...
        plt.grid()
        fig_file_name2 = os.path.abspath(os.path.join(os.path.dirname(__file__), 'TestLogs',
                                                      'Fig2_{0}.png'.format(datetime.now().strftime('%Y%m%d_%H%M'))))
        fig = plt.gcf()
        fig.savefig(fig_file_name2)
        plt.show()
        print (u'图表保存至：{0}'.format(fig_file_name2))
...
if __name__ == '__main__':
    ...
    from strategy.strategyMat import *
    ...
    # 在引擎中创建策略对象
    engine.initStrategy(MatStrategy, {})
