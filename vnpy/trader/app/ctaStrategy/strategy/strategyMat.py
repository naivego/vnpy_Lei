...
class MatStrategy(CtaTemplate):
    ...
    ccisArray = np.zeros(bufferSize)  # cci指标的数组
    ccibArray = np.zeros(bufferSize)  # cci指标的数组
    ...
    def onBar(self, bar):
        ...
        if self.vada1:
            ...
            self.ccisArray[0:self.bufferSize - 1] = self.ccisArray[1:self.bufferSize]
            self.ccibArray[0:self.bufferSize - 1] = self.ccibArray[1:self.bufferSize]
            ...
            lgcont14 = usr_d[-1] < 85 and usr_k[-1] - usr_d[-1] < 10 and self.usr_highArr[-2] < bar.high
            # lgcont14 = usr_k[-1] < 85 and usr_d[-1] < 80 and usr_k[-1] - usr_d[-1] < 15 and \
            #            bar.close < self.usr_ma1Arr[-1] * 1.015 and self.usr_ma1Arr[-1] < self.usr_ma2Arr[-1] * 1.015 and \
            #            self.usr_ma2Arr[-1] < self.usr_ma3Arr[-1] * 1.02 and self.usr_highArr[-2] < bar.high

            self.ccisArray[-1] = (bar.close < self.usr_ma1Arr[-1] * 0.985)
            lgcont2 = 1 and (self.ccisArray[-1] or self.ccisArray[-2] or self.ccisArray[-3]) and self.usr_highArr[
                -2] < bar.high
            # ------------------------
            ...
            stcont14 = usr_d[-1] > 15 and usr_d[-1] - usr_k[-1] < 10 and self.usr_lowArr[-2] > bar.low

            # stcont14 = usr_k[-1] > 15 and usr_d[-1] > 20 and usr_d[-1] - usr_k[-1] < 15 and \
            #            bar.close > self.usr_ma1Arr[-1] * 0.985 and self.usr_ma1Arr[-1] > self.usr_ma2Arr[-1] * 0.985 and \
            #            self.usr_ma2Arr[-1] > self.usr_ma3Arr[-1] * 0.98 and self.usr_lowArr[-2] > bar.low

            self.ccibArray[-1] = (bar.close > self.usr_ma1Arr[-1] * 1.015)
            stcont2 = 1 and (self.ccibArray[-1] or self.ccibArray[-2] or self.ccibArray[-3]) and self.usr_lowArr[
                -2] > bar.low
            ...
            if self.pos == 0:
                ...
                if (lgcont11 or lgcont12 or lgcont13) and lgcont14:
                    sdop = self.usr_highArr[
                               -2] * 1.002  # (self.usr_highArr[-2]- self.usr_lowArr[-2]) * 0.8  + self.usr_lowArr[-2]
                    sdsize = round(
                        self.ctaEngine.capital * self.s1fundpct / self.ctaEngine.Multiplier[self.vtSymbol] / sdop)
                elif lgcont2:
                    sdop = self.usr_highArr[-2] * 1.002  # bar.close + 0 * self.ctaEngine.Tick_Size[self.vtSymbol]
                    sdsize = round(
                        self.ctaEngine.capital * self.s2fundpct / self.ctaEngine.Multiplier[self.vtSymbol] / sdop)
                if sdsize > 0:
                ...
                if (stcont11 or stcont12 or stcont13) and stcont14:
                    sdop = self.usr_lowArr[
                               -2] * 0.998  # (self.usr_highArr[-2]- self.usr_lowArr[-2]) * 0.2  + self.usr_lowArr[-2]
                    sdsize = round(
                        self.ctaEngine.capital * self.s1fundpct / self.ctaEngine.Multiplier[self.vtSymbol] / sdop)
                elif stcont2:
                    sdop = self.usr_lowArr[-2] * 0.998  # bar.close - 0 * self.ctaEngine.Tick_Size[self.vtSymbol]
                    sdsize = round(
                        self.ctaEngine.capital * self.s2fundpct / self.ctaEngine.Multiplier[self.vtSymbol] / sdop)
                if sdsize > 0:
            ...
            # 持有空头仓位
                ...
                if sgn1spcont:
                    sdsp = self.usr_highArr[
                               -2] + atrval * 0.0  # bar.close + 0 * self.ctaEngine.Tick_Size[self.vtSymbol]
                    orderID = self.cover(sdsp, abs(self.pos), stop=True)
                    self.orderList.append(orderID)
...
if __name__ == '__main__':
    ...
    btconfig['Start_Date'] = '2016-05-01'
    btconfig['End_Date'] = '2017-06-30'
